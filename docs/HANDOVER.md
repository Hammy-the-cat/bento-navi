...
| 優先 | 項目 | 補足 |
|---|---|---|
| 0 | **Codemagic で再ビルド → TestFlight で確認** | 2026-08-10にAdMobを削除したため、**既存のTestFlightビルドは古い**。必ず新しいビルドで提出すること |
| 1 | **スクリーンショット** | 実機で5枚撮影 → 規定サイズ `1284×2778px` に加工。（v1.0はアプリ内に広告枠が無いので、どの画面でも可） |
| 2 | App Store Connect に掲載情報を入力 | `docs/appstore_listing.md` からコピペするだけ |
| 3 | プライバシー申告 | **`docs/appstore_privacy.md` の手順どおりに入力する**（画面のどこを押すかまで記載済み）。結論は「正確な位置情報／Appの機能／IDにリンクしない／トラッキングしない」の1項目だけ |
