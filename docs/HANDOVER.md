...
| 優先 | 項目 | 補足 |
|---|---|---|
| 1 | **スクリーンショット** | 実機で5枚撮影 → 規定サイズ `1284×2778px` に加工。広告枠が写らない画面を選ぶ |
| 2 | App Store Connect に掲載情報を入力 | `docs/appstore_listing.md` からコピペするだけ |
| 3 | プライバシー申告 | 「位置情報を収集・アプリの機能・ユーザーには紐づけない」で申告 |
| 4 | 配信地域を**日本のみ**に | EU配信にはデジタルサービス法の事業者登録が必要 |
| 5 | 審査に提出 | |
...
- **AdMob（アプリ内広告）は Google 公式のテストIDのまま**
  - 実IDに差し替える箇所は3つ: `lib/config/ad_config.dart` / `ios/Runner/Info.plist` の `GADApplicationIdentifier` / `android/.../AndroidManifest.xml`
  - ⚠️ **自分で実広告をタップすると規約違反（アカウント停止）**。動作確認はテストIDのまま行うこと
